	...
	public  function insertRow( $data , $id)
	{
	   ...
	   $key = with(new static)->primaryKey;
	    if($id == NULL )
        {
			...
			if($table!='tb_division' && $table!='tb_period'  && $table!='tb_teachers' && $table!='tb_parent' && $table!='tb_students' && $table!='tb_users' && $table!='tb_school'){
			...
			if($table!='tb_teachers' && $table!='tb_period' && $table!='tb_parent' && $table!='tb_students' && $table!='tb_users' && $table!='tb_school') {
				$data['year_id'] = \Session::get('selected_year');
			}
