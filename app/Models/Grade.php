        ...
        $year_id = \Session::get('selected_year');
         $result = \DB::select(
             "SELECT tb_grade.*,tb_subject.name As subject_name,tb_users.first_name As student_name,tb_class.name As class_name FROM tb_grade
              LEFT JOIN tb_subject ON tb_grade.subject_id=tb_subject.id JOIN tb_users ON tb_grade.student_id=tb_users.id
              LEFT JOIN tb_class ON tb_subject.class_id=tb_class.id
              WHERE tb_grade.student_id =".$studentId." AND tb_grade.year_id = '$year_id'"
              );
        return $result;
    }
}