...
class Controller extends BaseController
{
    ...
    public function getComboselectstudent(Request $request)
    {
        if($request->ajax() == true && \Auth::check() == true)
        {
            ...
            $table = $param[0];
            $rows =  \DB::table($table)->join('tb_users', "$table.user_id", '=','tb_users.id')->where($parent[0],$parent[1])->select("$table.student_id as student_id", \DB::raw('concat(tb_users.first_name, " ",tb_users.last_name) as name'))->get();
            $items = array();
