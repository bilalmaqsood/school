...
class SiteHelpers
{
    ...
    public static function showUploadedProfileIamge($file,$path , $class = '',  $width = 80, $height = 80) {
        ...
        if(file_exists($files ) && $file !="") {
            ...
            if($info['extension'] == "jpg" || $info['extension'] == "jpeg" ||  $info['extension'] == "png" || $info['extension'] == "gif" || $info['extension'] == "JPG")
            {
                $path_file = str_replace("./","",$path);
                return '
				<img src="'.asset( $path_file . $file ).'" width="'.$width.'" height="'.$height.'" class="'.$class.'"/>';
            } else {
                $path_file = str_replace("./","",$path);
                return '<p> <a href="'.url($path_file . $file).'" target="_blank"> '.$file.' </a>';
            }
    ...
    public static function avatar( $width =75)
    {
        if(empty($width))
        {
            $avatar = "http://www.gravatar.com/avatar/".md5(Session::get('email'));
            $Q = DB::table("customers")->where("email",'=',Session::get('eid'))->get();
            if(count($Q)>=1)
            {
                $row = $Q[0];
                $files =  './uploads/users/'.$row->image ;
                if($row->image !='' )
                {
                    if( file_exists($files))
                    {
                        return  URL::to('uploads/users').'/'.$row->image;
                    } else {
                        return $avatar;
                    }
                } else {
        ...
        $Q = DB::table("tb_users")->where("email",'=',Session::get('eid'))->get();
        if(count($Q)>=1)
        {
            $row = $Q[0];
